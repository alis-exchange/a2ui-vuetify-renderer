...
import { provide, shallowRef, onMounted, onUnmounted, watchEffect, computed } from 'vue';
...
provide(A2UI_CONTEXT_KEY, {
  get surfaceId() { return props.surfaceId; },
  get processor() { return props.processor; },
  onAction: (action: any) => {
    if (props.onAction) {
...
<template>
  <div :key="updateKey" class="a2ui-provider">
    <v-theme-provider v-if="activeThemeName" :theme="activeThemeName">
      <slot></slot>
    </v-theme-provider>
    <slot v-else></slot>
  </div>
